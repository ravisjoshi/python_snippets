...
class Solution:
    def distanceBetweenBusStops(self, distance, start, destination):
        ...
        if start > destination:
            ...
            cw = sum(distance[start:] + distance[:destination])
            acw = sum(distance[destination:start])

        else:
            cw = sum(distance[start:destination])
            acw = sum(distance[destination:] + distance[:start])
...
distance, start, destination = [7, 6, 3, 0, 3], 0, 4
print(s.distanceBetweenBusStops(distance, start, destination))
